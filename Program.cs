        ...
        if (!userResponses.ContainsKey(chatId))
{
    ...
    static async Task ProcessUserResponses(long chatId)
    {
        ...
        string[] prompts =
        {
            ...
            "💰 Введи сумму размена:",
            "📝 Добавь комментарий (если нужно), или напиши цифру или любую букву чтобы пропустить:"
        };
        ...
        if (responses.Count == 10)
        {
            ...
            await SaveToExcelAndSendReport(chatId, responses, revenue, cash, cashless, sbp, transfers, extra, exchange, income);
    ...
    static async Task SendLocationSelection(long chatId)
    {
        ...
        {
            new[] { InlineKeyboardButton.WithCallbackData("Грим Москва 1"), InlineKeyboardButton.WithCallbackData("Грим Москва 0") },
            new[] { InlineKeyboardButton.WithCallbackData("Грим Москва 3"), InlineKeyboardButton.WithCallbackData("Мультпарк") },
    ...
    static async Task SaveToExcelAndSendReport(long chatId, List<string> responses, decimal revenue, decimal cash, decimal cashless, decimal sbp, decimal transfers, decimal extra, decimal exchange, decimal income)
    {
        ...
        lock (fileLock)
        {
            ...
            if (row == 0)
            {
                ...
                sheet.Cells[1, 9].Value = "Доп. траты";
                sheet.Cells[1, 10].Value = "Размен";
                sheet.Cells[1, 11].Value = "Чистый доход";
                sheet.Cells[1, 12].Value = "Комментарий";
                row = 2;
            }
            else
            {
            ...
            sheet.Cells[row, 9].Value = extra;
            sheet.Cells[row, 10].Value = exchange;
            sheet.Cells[row, 11].Value = income;
            sheet.Cells[row, 12].Value = string.IsNullOrWhiteSpace(responses[9]) ? "-" : responses[9];
        ...
        if (!string.IsNullOrEmpty(location) && LocationChatIds.TryGetValue(location, out var locChatId))
        {
            string msg = $"🌸 *Твой милый отчёт готов, умничка!* 🌸" +
                         ...
                         $"🧾 *Доп. траты:* {extra} руб." +
                         $"💰 *Размен:* {exchange} руб." +
                         $"📝 *Комментарий:* {(string.IsNullOrWhiteSpace(responses[9]) ? "-" : responses[9])}";
